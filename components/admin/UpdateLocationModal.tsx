...
export default function UpdateLocationModal({ type, visible, closeHandler, location, buildings, departments }) {
  ...
  const handleUpdateLocation = async event => {
    ...
    let formData = getFieldsValues(event, ["id", "floor", "department_id", "building", "room_number"])
    if (other) {
  ...
  const handleCreateLocation = async event => {
    event.preventDefault();
    let formData = getFieldsValues(event, ["floor", "department_id", "building", "room_number"])
    if (other) {
      const tmp = getFieldsValues(event, ["building1"])
      formData.building = tmp.building1;
    }
      ...
      >
        ...
        <Modal.Body>
          <form onSubmit={(type == "create_location") ? handleCreateLocation : handleUpdateLocation} className="flex flex-col gap-4">
            ...
            <div className=" xl:w-88">
              <Text color="primary" className="mb-1">Department (Click to change department)</Text>
              <select defaultValue={location.department?.id}   disabled={(type == "view_details") ? true : false} name="department_id" className="form-select appearance-none block w-full p-2.5 px-5 text-base font-normal text-gray-700 border-2 rounded-2xl transition ease-in-out m-0
                  focus:text-gray-700 focus:bg-white focus:border-primary-color focus:outline-none" aria-label="building selection">
                <option>{location?.department? location?.department.name : "Department"}</option>
                {info.departments && info?.departments.map((department, id) => <option key={department.id} value={department.id} >{department.name}</option>)}
              </select>
            </div>
