        ...
        <Modal.Body>
          <form onSubmit={(type=="create_department")?handleCreateDepartment:handleUpdateDepartment} className="flex flex-col gap-4">
           ...
           <div className="w-2/3">
            ...
            <StyledStatus status={`${order.approval_by?.length > 0 && order.order_status == "Pending"? `Pending (${order.approval_by.length}/3)` : order.order_status}`} /> 
           </div>
          </div>
          ...
          <fieldset className="border-2 rounded-lg mb-4">
            <legend className="pl-1 pr-4 font-semibold">Items:</legend> 
              ...
              {order && order.order_items?.map((item, num) =>
                <div>
                    <Collapse key={num} title={`${item.description}`}>
              ...
              {(type=="view_details")
                ...
                :
                  <div className="flex justify-center gap-12">
                    ...
                    <Button auto className="bg-error-color" type="submit">Cancel Request</Button>
                  </div>
              }
            </div>
